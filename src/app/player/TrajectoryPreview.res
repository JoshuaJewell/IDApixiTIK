  ...
  if magnitude < 10.0 {
    ...
    while t.contents < Style.maxTime && pointIndex.contents < maxPoints {
      ...
      if py >= groundY {
        ...
        let flickerPhase = traj.timeAlive *. Style.pointFlickerSpeed +. Int.toFloat(pointIndex.contents)
        let sizeOscillation = Js.Math.sin(flickerPhase)
        let radius = Style.minPointRadius +.
