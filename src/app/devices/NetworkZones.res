...
let allZones: array<networkZone> = [
  // ========== EDGE NETWORKS (Local networks at different locations) ==========
  {
    ...
    canAccessZones: ["downtown-dmz", "downtown-internal", "downtown-dev", "downtown-iot", "downtown-mgmt", "isp-tier3-business", "public"],
  },
  {
    id: "rural-lan",
    name: "Rural Home LAN",
  ...
  },
  {
    ...
    canAccessZones: ["downtown-lan", "downtown-dmz", "downtown-internal", "downtown-dev", "downtown-iot", "downtown-mgmt", "downtown-scada"],
  },
  {
    id: "downtown-iot",
    name: "Downtown IoT",
  ...
  },
  {
    ...
    canAccessZones: ["downtown-lan", "downtown-dmz", "downtown-internal", "downtown-dev", "downtown-security", "downtown-iot", "downtown-scada"],
  },
  {
    id: "downtown-scada",
    name: "Downtown SCADA",
...
let canZoneAccessZone = (sourceZoneId: string, destZoneId: string): bool => {
  ...
  if sourceZoneId == destZoneId {
    ...
    | Some(sourceZone) => Array.includes(sourceZone.canAccessZones, destZoneId)
    | None => false
    }
  }
}
...
let canRouteViaISP = (sourceZone: networkZone, destZone: networkZone): bool => {
  ...
  if destZone.category == Service {
    ...
    let canReachISP =
      Array.includes(sourceZone.canAccessZones, "isp-tier3-business") ||
      Array.includes(sourceZone.canAccessZones, "isp-tier3-rural") ||
      Array.includes(sourceZone.canAccessZones, "isp-tier2-regional") ||
      Array.includes(sourceZone.canAccessZones, "isp-tier1-backbone")

    // If source can reach ISP infrastructure, it can reach public services
    canReachISP || Array.includes(sourceZone.canAccessZones, "public")
    ...
    Array.includes(sourceZone.canAccessZones, destZone.id) ||
    Array.includes(sourceZone.canAccessZones, "isp-tier3-business") ||
    Array.includes(sourceZone.canAccessZones, "isp-tier3-rural") ||
    Array.includes(sourceZone.canAccessZones, "public")
  } else {
...
let canIpAccessIp = (sourceIp: string, destIp: string): bool => {
  ...
  if sourceIp == destIp {
    ...
    | (Some(sZone), None) =>
      ...
      Array.includes(sZone.canAccessZones, "public") ||
      Array.includes(sZone.canAccessZones, "isp-tier3-business") ||
      Array.includes(sZone.canAccessZones, "isp-tier3-rural")
