...
let make = (~name: string, ~ipAddress: string, ~securityLevel: securityLevel, ~worldX: float=0.0, ()): t => {
  name,
  ipAddress,
  securityLevel,
  worldX,
...
let cameraStates: Dict.t<cameraState> = Dict.make()

let getCameraState = (ipAddress: string): cameraState => {
  switch Dict.get(cameraStates, ipAddress) {
  | Some(state) => state
  | None =>
    let state = { isRecording: true, isEnabled: true, isLooping: false, loopedMotionState: false, loopedHackerPosition: None }
    Dict.set(cameraStates, ipAddress, state)
    state
  }
}

let createCameraInterface = (container: Container.t, cameraWorldX: float, ipAddress: string): int => {
  ...
  for i in 0 to Int.fromFloat(feedHeight /. 4.0) {
    let _ = scanlines
  ...
  Graphics.on(loopBtn, "pointertap", _ => {
    ...
    if state.isLooping {
      ...
      Text.setText(header, if state.isRecording { "REC - LIVE FEED" } else { "PAUSED" })
      // Resume live feed
  ...
  if !state.isRecording {
    Text.setText(stopRecText, "START REC")
    Text.setText(header, if state.isEnabled { "PAUSED" } else { "DISABLED" })
    Container.setVisible(Graphics.toContainer(recIndicator), false)
  }
...
let openGUI = (device: t): DeviceWindow.t => {
  ...
  let intervalId = createCameraInterface(DeviceWindow.getContent(win), device.worldX, device.ipAddress)
