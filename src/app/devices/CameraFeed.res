...
let motionCallbacks: Dict.t<option<bool> => unit> = Dict.make()
...
let cameraPositions: Dict.t<(float, float)> = Dict.make()
...
let loopingCameras: Dict.t<bool> = Dict.make()
...
let updateAllCameraMotion = (): unit => {
  Dict.forEachWithKey(cameraPositions, (position, ip) => {
