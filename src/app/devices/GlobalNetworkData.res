...
let contentStore: Dict.t<globalFileContent> = Dict.make()
...
let updateContent = (~oldId: string, ~newContent: string): string => {
  ...
  | Some(old) => old.sizeMB
  | None => Float.fromInt(String.length(newContent)) /. 1024.0 /. 1024.0
  }
...
let clearContent = (): unit => {
  Dict.keysToArray(contentStore)->Array.forEach(key => Dict.delete(contentStore, key)->ignore)
}
