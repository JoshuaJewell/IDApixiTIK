...
module ServiceManager = {
  ...
  let runningServices: Dict.t<Dict.t<bool>> = Dict.make()

  // Get device services dict
  let getDeviceServices = (ip: string): Dict.t<bool> => {
  ...
  let getRunningServiceProcesses = (ip: string): array<(string, float, int)> => {
    ...
    Array.forEach(serviceDefinitions, def => {
      ...
      if isRunning {
        Array.push(runningProcs, (def.processName, def.cpuPercent, def.sizeGq))
      }
    })
...
let getTotalStorageUsage = (pm: processManager, ipAddress: string): (int, int) => {
  ...
  let fileUsageGq = Int.fromFloat(fileUsageMB /. 100.0)
  let fileUsageGq = if fileUsageGq < 1 { 1 } else { fileUsageGq }
...
let createLaptopState = (~ipAddress: string="192.168.1.102", ~hostname: string="WORKSTATION-PC", ~isServer: bool=false, ()): laptopState => {
