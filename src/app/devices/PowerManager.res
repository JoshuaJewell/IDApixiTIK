...
// Global power states
let powerStationStates: Dict.t<powerStationState> = Dict.make()
let upsStates: Dict.t<upsState> = Dict.make()

// Track which devices are shutdown due to power loss
let shutdownDevices: Set.t<string> = Set.make()

// Track which devices were manually shutdown by user (these don't auto-boot)
let manuallyShutdownDevices: Set.t<string> = Set.make()
...
let connectUPSToPowerStation = (upsIp: string, stationIp: string): unit => {
  ...
  if !Array.includes(station.connectedUPS, upsIp) {
...
let connectDeviceToUPS = (deviceIp: string, upsIp: string): unit => {
  ...
  if !Array.includes(ups.connectedDevices, deviceIp) {
...
let getDevicePowerSource = (deviceIp: string): powerSource => {
  ...
  let connectedUps = Dict.toArray(upsStates)->Array.find(((_, ups)) =>
    Array.includes(ups.connectedDevices, deviceIp)
  )
  ...
  | Some((upsIp, ups)) =>
    ...
    let connectedStation = Dict.toArray(powerStationStates)->Array.find(((_, station)) =>
      Array.includes(station.connectedUPS, upsIp)
    )
...
let updateUPSBatteries = (deltaSeconds: float): unit => {
  Dict.forEachWithKey(upsStates, (ups, _ip) => {
    if ups.isCharging {
      // Charging at 10% per minute when on mains
      let newLevel = ups.batteryLevel +. deltaSeconds *. (10.0 /. 60.0)
      ups.batteryLevel = if newLevel > 100.0 { 100.0 } else { newLevel }
    } else {
      ...
      ups.batteryLevel = if newLevel < 0.0 { 0.0 } else { newLevel }
    }
  })
}
...
let powerChangeCallbacks: Dict.t<bool => unit> = Dict.make()
...
let notifyPowerChange = (): unit => {
  Dict.forEachWithKey(powerChangeCallbacks, (callback, deviceIp) => {
