...
let createPowerStationInterface = (container: Container.t, ipAddress: string): unit => {
  ...
  // Header
  let headerStyle = {"fontFamily": "monospace", "fontSize": 16, "fill": 0xFFEB3B, "fontWeight": "bold"}
  ...
  let _ = statusIndicator
    ...
    ->Graphics.fill({"color": if state.isOnline { 0x00ff00 } else { 0xff0000 }})
  ...
  let statusLabel = Text.make({
    ...
    "style": {"fontSize": 18, "fill": if state.isOnline { 0x00ff00 } else { 0xff0000 }, "fontFamily": "monospace", "fontWeight": "bold"},
  })
  ...
  let fillWidth = if state.isOnline { 340.0 } else { 0.0 }
  let _ = meterFill
  ...
  let _ = toggleBtn
    ...
    ->Graphics.fill({"color": if state.isOnline { 0xff4444 } else { 0x44ff44 }})
  ...
  let toggleText = Text.make({
    "text": if state.isOnline { "SHUTDOWN" } else { "START" },
  ...
  Graphics.on(toggleBtn, "pointertap", _ => {
    ...
    let _ = statusIndicator
      ->Graphics.circle(50.0, 90.0, 20.0)
      ->Graphics.fill({"color": if newState { 0x00ff00 } else { 0xff0000 }})

    Text.setText(statusLabel, if newState { "ONLINE" } else { "OFFLINE" })

    // Update meter
    Graphics.clear(meterFill)->ignore
    let newFillWidth = if newState { 340.0 } else { 0.0 }
    let _ = meterFill
    ...
    let _ = toggleBtn
      ->Graphics.rect(20.0, btnY, 150.0, 40.0)
      ->Graphics.fill({"color": if newState { 0xff4444 } else { 0x44ff44 }})
      ->Graphics.stroke({"width": 2, "color": 0x000000})

    Text.setText(toggleText, if newState { "SHUTDOWN" } else { "START" })
