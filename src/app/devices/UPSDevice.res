...
let make = (~name: string, ~ipAddress: string, ~securityLevel: securityLevel, ~connectedStationIp: string, ()): t => {
...
let createUPSInterface = (container: Container.t, ipAddress: string): int => {
  ...
  // Header
  let headerStyle = {"fontFamily": "monospace", "fontSize": 16, "fill": 0x795548, "fontWeight": "bold"}
  ...
  let chargingLabel = Text.make({
    "text": if upsState.isCharging { "⚡ CHARGING" } else { "🔋 DISCHARGING" },
    "style": {"fontSize": 12, "fill": if upsState.isCharging { 0x00ff00 } else { 0xffaa00 }, "fontFamily": "monospace"},
  })
  ...
  let runtimeMinutes = if upsState.isCharging {
    ...
    if deviceCount > 0 {
      Int.fromFloat(upsState.batteryLevel /. (Int.toFloat(deviceCount) *. (1.0 /. 30.0) *. 60.0))
    } else {
      999
    }
  }
  ...
  let intervalId = setInterval(() => {
    ...
    Text.setText(chargingLabel, if currentState.isCharging { "⚡ CHARGING" } else { "🔋 DISCHARGING" })
