    ...
    if !nodeA.isFixed {
      Array.forEach(nodeArray, nodeB => {
        if nodeA.id != nodeB.id {
          ...
          if dist > 0.0 && dist < 400.0 { // Wide range for repulsion
            let force = if dist < minDist {
              strength *. 2.0 /. (dist +. 1.0) // Very strong if too close
            } else {
...
let tick = (sim: simulation, ~repulsionStrength: float, ~attractionStrength: float, ~minDist: float): unit => {
...
let runSimulation = (sim: simulation, maxIterations: int, ~repulsionStrength: float, ~attractionStrength: float, ~minDist: float): unit => {
  let i = ref(0)
