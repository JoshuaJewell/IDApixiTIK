    ...
    | _ =>
      ...
      Container.on(indicatorContainer, "pointertap", e => {
        ...
        | Some(feedData) =>
          ...
          let distance = Js.Math.abs_float(hackerX -. deviceX)
...
let buildLocationScreen = (location: LocationData.location): Navigation.appScreen => {
  ...
  let buildingCount = Int.fromFloat(location.worldWidth /. 200.0)
  for i in 0 to buildingCount {
    let bx = Int.toFloat(i) *. 200.0
    let bh = 100.0 +. Js.Math.random() *. 150.0
    let _ = buildings
      ->Graphics.rect(bx, groundY -. bh, 150.0, bh)
      ->Graphics.fill({"color": 0x252540})
    // Windows
    for wy in 0 to Int.fromFloat(bh /. 30.0) {
      for wx in 0 to 3 {
        let windowLit = Js.Math.random() > 0.5
        let _ = buildings
          ->Graphics.rect(
  ...
  Array.forEach(devices, wd => {
    Container.on(wd.container, "pointertap", _ => {
      ...
      let distance = Js.Math.abs_float(Player.getX(player) -. wd.x)
...

  {
    ...
    show: Some(async () => {
      ...
      %raw(`window.addEventListener('keydown', handler)`)
    ...
    hide: Some(async () => {
      ...
      | Some(handler) => %raw(`window.removeEventListener('keydown', handler)`)
      | None => ()
      }
    ...
    update: Some(ticker => {
      ...
      if !screenState.paused {
        ...
        if playerX > location.worldWidth -. 50.0 { Player.setX(player, location.worldWidth -. 50.0) }
        ...
        let clampedCameraX = Js.Math.max_float(minCameraX, Js.Math.min_float(maxCameraX, targetCameraX))
